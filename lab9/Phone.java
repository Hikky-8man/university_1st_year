...
public class Phone implements Comparable<Phone> {
  ...
  public int compareTo(Phone otherPhone) {
    ...
    //First it compares the date that the phone was released
    //IF they are the same, then compare the brand names
    int result;
    String[] announcedSplit;
    String[] otherAnnouncedSplit;
    announcedSplit=new String[2];
    otherAnnouncedSplit=new String[2];
    announcedSplit=this.announced.split(" ");
    //for (int i=0; i>=2; i++) {
    //  announcedSplit[i]=this.announced.split(" ")[i];
    //  otherAnnouncedSplit[i]=otherPhone.getAnnounced().split(" ")[i];
    //  System.out.println(announcedSplit[i]);
    //}
    otherAnnouncedSplit=otherPhone.getAnnounced().split(" ");
    //I got lazy at this point
    if (announcedSplit[0].equals(otherAnnouncedSplit[0])) {
      result=brand.compareTo(otherPhone.getBrand());
    } else {
      result = Integer.parseInt(announcedSplit[0]) - Integer.parseInt(otherAnnouncedSplit[0]);
    }
    return result;
  }
